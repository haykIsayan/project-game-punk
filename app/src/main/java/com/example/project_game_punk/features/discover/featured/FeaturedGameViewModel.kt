...
class FeaturedGameSuccessState(game: GameModel): ViewModelState.SuccessState<GameModel>(game) {
...
class FeaturedGameViewModel @Inject constructor(
...
): StateViewModel<GameModel, Unit>() {

    init {
        loadState()
    }

    fun trackUntrackGame(game: GameModel) {
    ...
    override suspend fun loadData(param: Unit?): GameModel {
